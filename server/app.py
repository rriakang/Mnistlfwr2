...
class FLServer():
    ...
    def get_eval_fn(self, model, model_name):
        ...
        def evaluate(
            ...
            if self.model_type == "Tensorflow":
                ...
                #하이퍼파라미터 정보 저장 (예 : config에서 lr, bs를 사용)
    ...
    def start(self):
        ...
        # 만약 hyperparameter 모드라면, 저장된 하이퍼파라미터 JSON 파일이 있으면 로드하여 처리
        if self.model_type == "hyperparameter":
            ...
            if os.path.exists(hyperparam_json):
                ...
                # 추가 초기화나 하이퍼파라미터 재설정을 여기에 수행할 수 있음

        # New Global Model Version: 마지막 버전 + 1
        self.server.gl_model_v = self.server.last_gl_model_v + 1

        # 서버 상태 전송용 정보 구성
        inform_Payload = {
            "S3_bucket": "fl-gl-model",  # 버킷 이름
            "Last_GL_Model": "gl_model_%s_V.h5" % self.server.last_gl_model_v,  # 이전 글로벌 모델 파일 이름
            "FLServer_start": today_time,
            "FLSeReady": True,           # 서버 준비 상태
            "GL_Model_V": self.server.gl_model_v  # 새 글로벌 모델 버전
        }
        ...
        try:
            ...
            fl_end_time = time.time() - fl_start_time  # FL 전체 운영 시간
            ...
            if self.model_type == "Tensorflow":
                ...
                # hyperparameter 모드에서는 Pytorch 방식과 유사하게 모델 파일 저장 및 업로드
                global_model_file_name = f"{gl_model_name}_gl_model_V{self.server.gl_model_v}.pth"
                server_utils.upload_model_to_bucket(self.task_id, global_model_file_name)
